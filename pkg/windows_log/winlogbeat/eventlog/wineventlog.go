...
import (
	...
	"d-eyes/pkg/windows_log/common/cfgwarn"
...
func init() {
	// Register winevtlog API if it is available.
...
// defaultWinEventLogConfig is the default configuration for new winevtlog readers.
...
func (l *winEventLog) openChannel(bookmark win.EvtHandle) error {
	...
	subscriptionHandle, err := win.Subscribe(
		...
		signalEvent,
		l.channelName,       // Channel - empty b/c channel is in the query
...
func (l *winEventLog) openFile(state checkpoint.EventLogState, bookmark win.EvtHandle) error {
	...
	h, err := win.EvtQuery(0, path, l.query, win.EvtQueryFilePath|win.EvtQueryReverseDirection)
	if err != nil {
...
func (l *winEventLog) eventHandles(maxRead int) ([]win.EvtHandle, int, error) {
	...
	if maxRead == 0 { // maxRead为零就说明, 按时间获取事件句柄, 获取到事件数量不以maxRead为准!
		var handles = make([]win.EvtHandle, 0, 200)
		for {
			hs, err := win.EventHandles(l.subscription, 10)
			if err != nil {
...
// It is a cut down version of the PublisherMetadataStore caching in winevtlog.Renderer.
